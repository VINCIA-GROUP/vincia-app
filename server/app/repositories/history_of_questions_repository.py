...
class HistoryOfQuestionsRepository(Repository):
   ...
   def get_by_id(self, id, user_id):
      return super().get_one(
         query="SELECT q.id, q.create_at, q.answer_at, q.hit_level, q.time, q.question_id, q.user_id, calculate_rating FROM history_of_questions q WHERE q.id = %s AND q.user_id = %s;", 
         params=(id,user_id)
      )
   ...
   def get_history_without_answer(self, user_id):
      return super().get_one(
         query="SELECT q.id, q.create_at, q.answer_at, q.hit_level, q.time, q.question_id, q.user_id, calculate_rating FROM history_of_questions q WHERE q.user_id = %s AND q.answer_at IS NULL;",
         params=(user_id,)
      )
